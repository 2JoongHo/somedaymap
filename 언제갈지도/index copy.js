...
window.onload = function () {
  ...
  kakao.maps.event.addListener(map, 'click', function (mouseEvent) {
    ...
    // 현재 열려 있는 '내 장소' 인포윈도우가 있다면 닫기
    if (currentOpenInfowindow) {
      currentOpenInfowindow.close();
      currentOpenInfowindow = null;
    }
    // 모든 검색 결과 인포윈도우 닫기
    searchInfowindows.forEach(infowindow => infowindow.close());

    // ✨✨✨ 새로 추가되는 부분: 클릭한 좌표 주변의 상호명 검색 ✨✨✨
    // 키워드 없이 좌표와 반경으로 주변 장소 검색을 요청
    ps.keywordSearch('', searchPlaceByMapClickCB, {
      x: latlng.getLng(), // 클릭한 경도
      y: latlng.getLat(), // 클릭한 위도
      radius: 100, // 100m 이내의 장소 검색
      sort: kakao.maps.services.SortBy.DISTANCE // 가까운 순으로 정렬
    });
    // ✨✨✨ 새로 추가되는 부분 끝 ✨✨✨
  });
